...
export class ResizeDirective implements AfterViewInit {
  ...
  mouseDown = (event: MouseEvent) => {
    ...
    const listenerLeave = this.renderer.listen(
      theadRow,
      "mouseleave",
      this.endDrag
    );
    ...
    this.index = parseInt(
      `${(resize as HTMLDivElement).getAttribute("th-resize")}`
    );
    ...
    this.width = parseInt(
      getComputedStyle(
        this.isFixed ? this.nextTh : this.th,
        null
    ...
    if (this.nextTh != undefined && !this.isFixed) {
      this.nextWidth = parseInt(
        getComputedStyle(this.nextTh, null).getPropertyValue("width"),
        10
      );
    }
  };
  ...
  firstThLimit() {
    const leftPd = parseInt(
      getComputedStyle(this.th, null).getPropertyValue("padding-left"),
      10
    );
    const rightPd = parseInt(
      getComputedStyle(this.th, null).getPropertyValue("padding-right"),
      10
    );
    const offset = leftPd + rightPd;
    return (
      Math.round(this.thCollection[0].getBoundingClientRect().width) - offset >
      60
    );
  }
  ...
  mouseMove = (event: MouseEvent) => {
    if (this.dragStart) {
      ...
      const newWidth = this.isFixed
        ? this.width - mouseMoved
        : this.width + mouseMoved;
